...
export default function Stackingspend() {
    const essentialData = dashboardStats.timeSeries.map(item => item.essential);
    const luxuryData = dashboardStats.timeSeries.map(item => item.luxury);
    const investmentData = dashboardStats.timeSeries.map(item => item.investment);
        ...
        <div className="w-full h-full">
            <div className="bg-card p-4 rounded-2xl border border-divider shadow-sm h-full flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-[11px] font-bold text-text-secondary uppercase tracking-widest leading-none">Expenses</h3>
                ...
                <Box sx={{ width: '100%', height: 320, flex: 1 }}>
                    <BarChart
                        height={300}
                        series={[
                            { data: essentialData, label: 'Essential', stack: 'total', color: '#10B981' },
                            { data: luxuryData, label: 'Luxury', stack: 'total', color: '#3B82F6' },
                            { data: investmentData, label: 'Investment', stack: 'total', color: '#F59E0B' },
                        ]}
